...
function renderHolidays(list){
  ...
  parsed.slice(0,6).forEach(h => {
    ...
    const daysAway = Math.ceil((h.dt - now) / (1000*3600*24));
    const right = document.createElement("div");
    right.className = "small-text";
    right.textContent = daysAway >= 0 ? `${daysAway} day(s)` : "passed";
    li.appendChild(left);
...
function weatherEmojiFromCode(icon, main){
  ...
  // fallback by main
  const map = {
    Clear: "☀️", Clouds: "☁️", Rain: "🌧️", Snow: "❄️", Thunderstorm: "⛈️", Drizzle: "🌦️", Mist: "🌫️"
  };
  return map[main] || "🌤️";
}
